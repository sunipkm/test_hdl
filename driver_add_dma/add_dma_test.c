...
static struct add_dma_channel_interface *tx_proxy_interface_p;
...
void *tx_thread(void *dma_count)
{
	...
	for (counter = 0; counter < *((int *)dma_count); counter++) {
...
{
	struct add_dma_channel_interface *rx_proxy_interface_p;
	int rx_proxy_fd, i;
	int dummy;
	int counter;
	pthread_t tid;
	...
	/* Open the DMA proxy device for the transmit and receive channels
 	 */
	tx_proxy_fd = open("/dev/add_dma_out", O_RDWR);
	...
	rx_proxy_fd = open("/dev/add_dma_in", O_RDWR);
	if (tx_proxy_fd < 1) {
		printf("Unable to open DMA proxy device file");
		exit(EXIT_FAILURE);
	}
	...
	/* Map the transmit and receive channels memory into user space so it's accessible
 	 */
	tx_proxy_interface_p = (struct add_dma_channel_interface *)mmap(NULL, sizeof(struct add_dma_channel_interface),
									PROT_READ | PROT_WRITE, MAP_SHARED, tx_proxy_fd, 0);

	rx_proxy_interface_p = (struct add_dma_channel_interface *)mmap(NULL, sizeof(struct add_dma_channel_interface),
	 ...
	 */
	munmap(tx_proxy_interface_p, sizeof(struct add_dma_channel_interface));
	munmap(rx_proxy_interface_p, sizeof(struct add_dma_channel_interface));
